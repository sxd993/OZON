...
async def main(query: str, max_products: int, output_file: str, progress_handler) -> None:
    ...
    try:
        ...
        products_urls_list = page_down(driver=driver, css_selector="a[href*='/product/']", colvo=max_products)
        ...
        collect_data(products_urls=products_urls, driver=driver, progress_handler=progress_handler)
...
if __name__ == "__main__":
    import asyncio
    asyncio.run(main(query="смартфон", max_products=1000, output_file="ozon_products.xlsx", progress_handler=None))